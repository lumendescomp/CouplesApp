...
router.get("/", (req, res) => {
  ...
  const items = db
    .prepare(
      "SELECT id, item_key, x, y, z, rotation FROM couple_items WHERE couple_id = ? ORDER BY id"
    )
    .all(cpl.id);
...
router.post("/items", (req, res) => {
  ...
  const { item_key, x = 50, y = 50, z = 0, rotation = 0 } = req.body || {};
  if (!item_key) return res.status(400).send("item_key required");
  const stmt = db.prepare(
    "INSERT INTO couple_items (couple_id, item_key, x, y, z, rotation) VALUES (?, ?, ?, ?, ?, ?)"
  );
  const info = stmt.run(
    cpl.id,
    String(item_key),
  ...
  const row = db
    .prepare(
      "SELECT id, item_key, x, y, z, rotation FROM couple_items WHERE id = ?"
    )
...
router.post("/items/:id/nudge", (req, res) => {
  ...
  const current = db
    .prepare(
      "SELECT id, item_key, x, y, z, rotation FROM couple_items WHERE id = ? AND couple_id = ?"
    )
    .get(id, cpl.id);
  ...
  const nx = clamp(Math.round(current.x + dx), 0, 100);
  const ny = clamp(Math.round(current.y + dy), 0, 100);
...
router.post("/items/:id/position", (req, res) => {
  ...
  const row = db
    ...
    .get(id, cpl.id);
  if (!row) return res.status(404).send("not found");
  const x = Math.max(0, Math.min(100, Math.round(Number(req.body.x || 0))));
  const y = Math.max(0, Math.min(100, Math.round(Number(req.body.y || 0))));
